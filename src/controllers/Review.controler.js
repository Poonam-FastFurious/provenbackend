...
const addReview = asyncHandler(async (req, res) => {
      try {
            ...
            if (![productName, name, email, rating, message].every((field) => field?.trim())) {
                  throw new ApiError(400, "Product name, name, email, rating, and message are required");
            }
...
const getReviewsByProductId = asyncHandler(async (req, res) => {
      try {
            ...
            const reviews = await Review.find({ productName: productId }).populate('productName', 'name'); // Adjust 'name' as needed
